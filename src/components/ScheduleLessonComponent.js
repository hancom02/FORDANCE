...
const ScheduleLessonComponent = props => {
  ...
  const [editedEmail, setEditedEmail] = useState(
    offlinelessons?.instructor.email,
  );
  const [editedPhoneNumber, setEditedPhoneNumber] = useState(
    offlinelessons?.instructor.phone,
  );
  ...
  const handleSave = () => {
    ...
    handleSubmit({
      ...
      endDate: editedEndDate ? editedEndDate : null,
    });
  ...
  return (
    <SafeAreaView style={styles.popUpFormContainer}>
      ...
      <ScrollView style={{flexGrow: 1}}>
        ...
        <Text style={styles.infoNameText}>{offlinelessons?.name}</Text>
        <View style={styles.infoContainer}>
          <Text style={styles.infoLabelText}>Instructor:</Text>
          <Text style={styles.infoText}>{offlinelessons?.instructor.name}</Text>
        </View>
        <View style={styles.infoContainer}>
        ...
        <View style={styles.infoContainer}>
          ...
          {editingMode ? (
            <TouchableOpacity onPress={() => setShowEndDatePicker(true)}>
              <Text style={styles.infoText}>
                {editedEndDate ? formatDate(editedEndDate) : 'Select end date'}
              </Text>
            </TouchableOpacity>
          ) : (
            <Text style={styles.infoText}>
          ...
          {showEndDatePicker && (
            <DateTimePicker
              value={new Date(editedEndDate) || new Date()}
              mode="date"
              display="default"
      ...
      <TouchableOpacity
        ...
        onPress={editingMode ? handleSave : handleEdit}>
