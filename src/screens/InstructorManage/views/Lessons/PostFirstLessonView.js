...
import createLesson from '../../../../api/lesson/create';
import {CATEGORIES, LEVELS} from '../../../../constants/options';
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (const fileDetail of selectedVideo) {
        ...
        videoUrl = await uploadVideoToS3(fileName, fileData);
      ...
      if (newThumbnail) {
        ...
        imageUrl = await uploadImageToS3(thumbnailFileName, thumbnailData);
      ...
      mutate({
        ...
        videoUrl,
        imageUrl,
        level: LEVELS.find(l => l.value == valueLevel)?.label,
        category: CATEGORIES.find(c => c.value == valueCategories)?.label,
      });
    } catch (error) {
          ...
          <View style={styles.chooseVideoContainer}>
            {thumbnail ? (
              ...
              <View>
                ...
                <TouchableOpacity
                  style={styles.chooseVideoButton}
                  onPress={handleChooseVideo}>
                  <Text style={styles.buttonText}>Choose Video</Text>
                </TouchableOpacity>
              </View>
          ...
          {thumbnail ? (
            <View>
              <View style={styles.chooseVideoContainer}>
                <ImageBackground
                  resizeMode="contain"
                  source={{uri: newThumbnail ? newThumbnail : thumbnail}}
                  style={styles.thumbnail}></ImageBackground>
              </View>
              {newThumbnail ? (
                <View
                  style={{
